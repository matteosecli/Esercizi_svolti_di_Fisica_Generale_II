...
\begin{proof}
	\begin{enumerate}
		...
		$$
		\boxed{y(t) = \frac{E}{B_0}\left( t - \frac{\in(\omega t)}{\omega} \right)};
		\qquad
...
\begin{problema}[$50$]
	...
	Essa è attraversata da una densità di corrente $j_{0}\,\hat{k}\,[Am^{2}]$. Determinare il 
...
\begin{problema}[$51$]
	Un dipolo magnetico $\vec{\mu} = \mu\,\hat{k}$ è situato sull'asse di una 
...
\begin{proof}
	\begin{enumerate}
		...
		\begin{align}
		\label{eq: forza}
		\vec{F} = \vec{m}\frac{\partial \vec{B}}{\partial z}
		\end{align}
...
\begin{proof}
	\begin{enumerate}
		...
		x^{2}\arctan\left(\frac{y}{x}\right) \right]_{0}^{a} \\
		&= \frac{\mu_{0} j_{0}}{a\pi}\left[
		a + x^{2}\arctan\left(\frac{a}{x}\right)\right]
		\end{align*}		
		Si ottiene allora:
		\begin{align*}
		\boxed{\vec{B}= \frac{\mu_{0} j_{0}}{a\pi}\left[
		a + x^{2}\arctan\left(\frac{a}{x}\right)\right]\hat{y}}	
		\end{align*}
...
\begin{proof}
	...
	\end{align*}
	dove $\hat{\phi}$ rappresenta il versore tangente 
