...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item supporre che i centri dei due cerchi siano entrambi sull'asse $x$, senza perdere di generalità.
	\end{enumerate}
...
%\begin{problema}
%	La piastra mostrata in figura (FIGURA) ha una superficie infinitamente grande ed uno spessore 
%	finito $d\,\mathrm{(m)}$. Essa è attraversata da una densità di corrente 
%	$j_0\hat{k}\,\mathrm{(A/m^{2})}$. Determinare il campo magnetico generato dalla corrente in 
%	tutto lo spazio.
%\end{problema}
%\begin{proof}
%	Non conosciamo le caratteristiche del materiale di cui è costituita la piastra, quindi ci limiteremo a calcolare il campo magnetico al suo esterno. Possiamo vedere la piastra come una successione di lamine di spessore infinitesimo, in cui scorre una corrente di densità lineare
%	$j_l = j_0dy\hat{k}$. Il campo generato da una singola lamina -- già calcolato in precedenza col Teorema di Ampère -- ha modulo
%	$$
%	dB = \frac{\mu_0j_l}{2} = \frac{\mu_0j_0dy}{2}.
%	$$
%	Per trovare il campo totale, basta integrare in $dy$ su tutto lo spessore $d$:
%	$$
%	B = \int_{-d/2}^{d/2} dB = \int_{-d/2}^{d/2} \frac{\mu_0j_0dy}{2}
%	= \boxed{\frac{\mu_0j_0d}{2}}.
%	$$
%	Al solito, il campo sarà concorde all'asse $x$ per $y<-d/2$, discorde per $y>d/2$.
%\end{proof}
...
\begin{proof}
	\begin{enumerate}
		...
		Dividiamo la piattina in fili infiniti attraversati da 
		corrente $i = j_{0}\frac{y}{a} dy$. Per ragioni di simmetria sarà 
...
\begin{problema}
	Nel circuito in figura i condensatori sono inizialmente
...
\begin{proof}
	...
	due maglie in figura e del nodo ($\vec{inserire una lettera?}$): 
	\begin{align*}
	\begin{cases}
	V_{1} = I_{1}R	 \\
	V_{2} = I_{1}R + I_{2}R \\
	...
	la definizione di corrente $I = \frac{dq}{dt}$ e il fatto che venga
	...
	\begin{cases}
	I_{1} = \frac{V_{1}}{R}	 \\
	I_{1} = \frac{V_{2}}{R} +  C\frac{dV_{2}}{dt}\\
	I_{1} = -C\frac{dV_{2}}{dt} - RC\frac{dI_{1}}{dt}
	\end{cases}
	\end{align*}
	Osservando inoltre che : 
	$I_{1} = -I_{2} + \frac{V_{2}}{R} 
	= C\frac{dV_{2}}{dt} + \frac{V_{2}}{R}$
...
\begin{problema}
	Gli elementi circuitali del circuito in figura 
